...
unsigned char resultL = 0;		// результат преобразования модулем АЦП
unsigned char resultH = 0;		// результат преобразования модулем АЦП
unsigned char counterTx = 0;		
	...
	while(1)
	{
		...
		{						// да
			if (UCSRA & (1<<UDRE) != 0) // регистр данных пуст?
			{
				...
				if (counterTx = 0)
				{
					UDR = resultL;		// запись данных из resultL в регистр передачи
					adcNewData = 0;		// сброс флага adcNewData
				}
				else
				{
					UDR = resultH;		// запись данных из resultH в регистр передачи
				}
			}
		}
	}
...
void InitADC(void)
{
	ADMUX = (1<<REFS1)|(1<<REFS0);	// опорн. напр. - напр питания,
...
ISR(ADC_vect)
{
	resultL = ADCL;		// чтение младшего регистра данных
	resultH = ADCH;		// чтение старшего регистра данных
