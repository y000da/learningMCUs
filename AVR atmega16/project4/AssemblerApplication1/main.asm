...
Init_B: ldi temp,0b11001111 ;PB4 PB5-вход,остальные выходы
	out DDRB,temp
	ldi temp,0b00110001 ;PB4 PB5 подт.резистор, разрешена работа 
...
Init_CNT: clr counter;установка счётчика в 0

;***********************************
;Начало основного цикла
;***********************************
Start: sbic PinB,4 ;проверка условия, что нажата кнопка
	rjmp PB5_check ;если нет, то переход на метку 
	inc counter ;если да, то инкремент счетчика counter
	rjmp check ;если нет, то переход на метку 

PB5_check:sbic  PinB,5    ;проверка условия, что нажата кнопка
		  rjmp  Start     ;если нет, то переход на метку Start
		  dec   counter   ;если да, то инкремент счетчика Counter

Check:    cpi   counter,16;проверка, что Counter=16?
	      brne  PC+2      ;если нет, то пропуск следующей команды
	      ldi   counter,0 ;если да, то установка счетчика Counter в 0
		  cpi   counter,-1;проверка, что Counter=-1 (или 255)?
	      brne  PC+2      ;если нет, то пропуск следующей команды
	      ldi   counter,15;если да, то установка счетчика Counter в 15 
;	   
...
Key_end1: sbis PinB,4 ;проверка условия, что кнопка отпущена
	rjmp Key_end1 ;если нет, то переход на метку Key_end
Key_end2: sbis PinB,5 ;проверка условия, что кнопка отпущена
	rjmp Key_end2 ;если нет, то переход на метку Key_end
